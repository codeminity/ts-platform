...
- [ADR-012: `onRefreshFail` retry is a callback capability, not a config policy](#adr-012-onrefreshfail-retry-is-a-callback-capability-not-a-config-policy)
...
## ADR-012: `onRefreshFail` retry is a callback capability, not a config policy

**Context:** `handleRefreshToken` called `refreshToken()` exactly once; a transient failure (a network blip, not an actually-invalid refresh token) failed the whole refresh cycle immediately, even though the original request has a full retry engine available to it. Competitive research turned up no equivalent in any comparable package, making this an internal-consistency argument rather than a market-parity one. The first design considered mirrored `RetryConfig`'s shape: new `refreshRetries`/`refreshRetryDelay`/`shouldRetryRefresh` fields on `AuthConfig`, with the framework owning a declarative retry loop. On review, this added three new config fields for a decision (whether a specific failure is worth retrying) that only the caller can actually make correctly — `request-core` has no idea what "transient" means for an arbitrary `refreshToken` implementation, and guessing via error shape would mean inventing protocol-specific classification core explicitly avoids elsewhere (see ADR-007).

**Decision:** `onRefreshFail` gains a second parameter, `retry: () => Promise<void>`, instead of any new config fields. Calling and `await`ing `retry()` re-runs the exact same attempt (including the `refreshTimeout` race, if configured) and resolves or rejects based on that attempt's real outcome — not a synchronous flag. Not calling `retry()` (the default — an existing `onRefreshFail: (error) => void` implementation is unaffected, since JS allows calling a function with more arguments than it declared) fails the refresh immediately, identical to today's behavior. There is no built-in retry limit; a caller that wants one tracks its own attempt count via closure state, the same way `onRefreshFail` itself is a plain function a caller already controls.

**Consequences:** Zero new public config surface for a feature that's fundamentally about caller judgment, not framework policy — the entire decision of "is this worth retrying, how many times, how long to wait between attempts" lives in application code that actually knows what its `refreshToken` implementation's failures mean. The implementation is a small recursive attempt loop (`packages/request/core/src/auth/refresh-token.ts`) rather than an iterative one specifically so `retry()`'s returned promise reflects the real nested outcome — an earlier iterative sketch that just set a boolean flag and let an outer loop retry later would make `await retry()` resolve before the real attempt finished, which is worse than not offering `retry()` at all: it would look like it works but silently lie about timing. The one accepted misuse case — a caller invokes `retry()` without awaiting or returning it — is documented, not prevented: the retried attempt still runs for real in the background (an internal `.catch(() => {})` only stops it from crashing the process as an unhandled rejection if it later fails), but the outer refresh cycle resolves without waiting for it, silently assuming success. This is treated the same as any other un-awaited promise in application code — the caller's responsibility, not something `request-core` should coerce them out of doing wrong.
